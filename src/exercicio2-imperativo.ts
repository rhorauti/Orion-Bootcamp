...
const pessoasImperativo: Array<Pessoa> = lista as Array<Pessoa>;

// a) Crie uma função que retorne a bio do id passado

function retornarBioImperativo(idx:number):string|undefined {
...
function retornarNameImperativo(idx:number):string|undefined {
    let isIdxValido:boolean = false;
    const tamanhoArray = pessoasImperativo.length;
    for(let i=0; i<tamanhoArray; i++) {
        if(pessoasImperativo[i].id == idx) {
...
function apagarItemImperativo(idx:number) {
    let isIdxValido:boolean = false;
    const tamanhoArray = pessoasImperativo.length;
    for(let i=0; i<tamanhoArray; i++) {
        if(pessoasImperativo[i].id == idx) {
...
function alterarBioOuNameImperativo(idx:number, bio:string = '', name:string = '') {
    let isIdxValido:boolean = false;
    const tamanhoArray = pessoasImperativo.length;
    for(let i=0; i<tamanhoArray; i++) {
        if(pessoasImperativo[i].id == idx) {
            ...
            if(bio!='' && name!='') {bioSelecionada = bio; nameSelecionado = name;}
            else if(bio != '') {bioSelecionada = bio;}
            else if(name != '') {nameSelecionado = name;}
            isIdxValido = true;
        } 
    }
    if(!isIdxValido) {
